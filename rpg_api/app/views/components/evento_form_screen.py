...
class PropriedadesEventoFormScreen(ModalScreen[dict]):
    ...
    def compose(self):
        ...
        with Vertical(id="evt-caixa-full"):
            ...
            with Horizontal(classes="linha-dupla"):
                ...
                yield Input(
                    value=self.dados_existentes.get("nome", f"ev_{self.linha}_{self.coluna}"),
                    id="evt-nome")
                yield Label("Emoji:", classes="campo-rotulo")
                yield Select(
                    [(v, v) for v in coletanea_emoji] + [(self.emoji, self.emoji)],
    ...
    def restaurar_valores_dos_campos(
        ...
        self.query_one("#input-linha-teleporte").value = str(linha_coletada)
        self.query_one("#input-coluna-teleporte").value = str(coluna_coletada)
